...
namespace EngineDemo {
	...
	public class Orbitter : MonoBehaviour {
		...
		public Tween IntroTween() {
			...
			Interactable = false;
			seq.Join(_parent.DORotate(_rotation, animTime).SetEase(Ease.InOutSine));
			seq.Join(transform.DOLocalMove(new Vector3(0.0f, 0.0f, _distance), animTime).SetEase(Ease.InOutCubic));
			seq.OnUpdate(() => {
